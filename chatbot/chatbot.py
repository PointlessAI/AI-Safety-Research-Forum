...
class ChatBot:
    def __init__(self, personality_name: str, personality_dir: Optional[str] = None):
        ...
        success = self.personality_manager.load_personality(personality_name)
        if not success:
            raise ValueError(f"Failed to load personality: {personality_name}")
            
        # Set up relationship manager with the personality directory
        if personality_dir is None:
            personality_dir = os.path.join(self.personality_manager.base_dir, "ai", personality_name)
        self.relationship_manager = RelationshipManager(personality_dir)
            
        print(f"✅ Loaded personality: {personality_name}")

    def _update_personalities(self, message1: str, message2: str, speaker1: str, speaker2: str) -> None:
        """Update both AI personalities based on the conversation."""
        conversation_segment = [
            {"speaker": speaker1, "listener": speaker2, "message": message1},
            {"speaker": speaker2, "listener": speaker1, "message": message2}
        ]
        
        # Update both personalities
        self._update_personality_files(speaker1, speaker2, conversation_segment)
        self._update_personality_files(speaker2, speaker1, conversation_segment)
